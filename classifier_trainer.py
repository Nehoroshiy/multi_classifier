...
from manopt.approximator import TangentVector
from manopt.approximator import gradient_projection_partial, svd_retraction
...
class NeuralNetworkTrainer(object):
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in xrange(num_iters):
            ...
            for p in model:
                if p.startswith('W'):
                    grads[p] = TangentVector(model[p], gradient_projection_partial(model[p], grads[p]))
                if update == 'sgd':
                    if p.startswith('W'):
                        dx = -learning_rate * grads[p].release()
                    else:
                        dx = -learning_rate * grads[p]
                elif update == 'momentum':
                    if not p in self.step_cache:
                        if p.startswith('W'):
                            self.step_cache[p] = ManifoldElement.zeros(grads[p].shape, ranks[p])
                        else:
                            self.step_cache[p] = np.zeros_like(model[p])
                    v_prev = self.step_cache[p]
                    if p.startswith('W'):
                        self.step_cache[p] = momentum * self.step_cache[p] - learning_rate * grads[p].release()
                    else:
                ...
                model[p] += dx
                if p.startswith('W'):
                    model[p] = svd_retraction(model[p], ranks[p])
        ...
        for it in xrange(num_iters):
            ...
            for p in model:
                ...
                if update == 'sgd':
                    ...
                    #####################################################################
                    self.step_cache[p] = 1. * momentum * self.step_cache[p] / (it + 1) - learning_rate * grads[p]
                    dx = self.step_cache[p]  # integrate position
                    #####################################################################
                    #                      END OF YOUR CODE                             #
                    #####################################################################
                elif update == 'rmsprop':
                    decay_rate = 0.9  # you could also make this an option
                    if not p in self.step_cache:
                        self.step_cache[p] = np.zeros(grads[p].shape)
                    #####################################################################
