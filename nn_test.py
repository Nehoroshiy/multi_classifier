...
from classifier_trainer import NeuralNetworkTrainer
...
if __name__ == '__main__':
    ...
    loss, _ = two_layer_net(X, model['model'], y, reg=0)

    # Sanity check: Loss should be about log(10) = 2.3026
    print 'Sanity check loss (no regularization): ', loss

    # Sanity check: Loss should go up when you add regularization
    loss, _ = two_layer_net(X, model['model'], y, reg=1)
    ...
    best_model, loss_history, train_acc_history, val_acc_history = trainer.train(
            ...
            reg=0.001, momentum=0.9, learning_rate=0.0001, batch_size=10, num_epochs=20,
            verbose=True)